        ...
        for(let i in this.props.result){
           result.push(
               <div className='search-result-block'>
                   ...
                   <div className='search-page-addFriend-btn-block'><button className='btn btn-success'>Add as Friend</button></div>
                   </div>
                   <div>
               <NavLink key={i} to={(endPoints.anotherUserPage.replace(':userId',this.props.result[i].userId))}>
                   <h5><span>{this.props.result[i].firstName} {this.props.result[i].middleName} {this.props.result[i].lastName}</span></h5></NavLink>
                   </div>
